...
data "terraform_remote_state" "vpc" {
  ...
  config = {
    bucket = "tfstate-dev-us-east-1-lqvonw"                             # Name of the remote S3 bucket where the VPC state is stored
    key    = "tf-state-deploy-env/${terraform.workspace}/vpc/dev/terraform.tfstate" # Path to the VPC tfstate file within the bucket

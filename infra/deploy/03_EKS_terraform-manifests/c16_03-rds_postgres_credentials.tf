...
locals {
  retailstore_secret_json = try(
    jsondecode(data.aws_secretsmanager_secret_version.retailstore_secret_value.secret_string),
    {}
  )
}
...
output "debug_retailstore_secret_dbuser" {
  ...
  value       = local.retailstore_secret_json.DB_USER
  sensitive   = true
}
...
#   description = "⚠️ For testing only: DB password from Secrets Manager"
#   value       = local.retailstore_secret_json.DB_PASS
#   sensitive   = true
# }
